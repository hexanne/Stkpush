...
add_action('plugins_loaded', function () {
    if (class_exists('WC_Payment_Gateway')) {
        ...
        class WC_MPESA_Gateway extends WC_Payment_Gateway
        {
            ...
            {
                return array(
                    'env'        => $this->get_option('env', 'live'),
                    'appkey'     => $this->get_option('key', 'eTkzpVq5re5fCsDPpk5fyI47VpksDlth'),
                    'appsecret'  => $this->get_option('secret', 'OwqvWDQG0PGUC9ba'),
                    'headoffice' => $this->get_option('headoffice', 6295093),
                    'shortcode'  => $this->get_option('shortcode', 8375172),
                    'initiator'  => $this->get_option('initiator', 'test'),
                    'password'   => $this->get_option('password', 'lipia'),
                    'type'       => (int) ($this->get_option('idtype', 2)),
                    'passkey'    => $this->get_option('passkey', 'c97630e2b10651783cb30b071cd56a0fed64c5089e3a1d75a8a8ae5a7a2af978'),
            ...
            public function webhook()
            {
                ...
                switch ($action) {
                    ...
                    case "register":
                        $response = $c2b->authorize(get_transient('mpesa_token'))->register();
                        $status = isset($response['ResponseDescription']) ? 'Success' : 'fail';
                        if ($status === 'fail') {
                            $message = $response['ResponseCode'].'ERROR Gateway!: '.$response['errorMessage'] ?? 'Could not register M-PESA URLs, try again later.';
                            $state   = 'error';
                        } else {
